...
First, run an ordinary regression of $y_t$ on $z_{t1}$, ··· , $z_{tr}$ (acting as if the errors are uncorrelated). Retain the residuals, $\hat{x}_{t} = y_t-\sum^{r}_{j=1}\beta_{j} z_{tj}$.
